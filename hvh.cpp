...
void HVH::AntiAim() {
	...
	if (!g_menu.main.aimbot.enableaa.get()) {
		m_should_work = false;
		return;
	}
	...
	if (g_menu.main.aimbot.desync.get()) { // won't fuck up even if you're shooting
	...
	if ((g_cl.m_local->m_fFlags() & FL_ONGROUND) && g_input.GetKeyState(g_menu.main.aimbot.fakeduck.get())) {
		m_fakeduck = true;
	...
	if (g_menu.main.aimbot.pitch.get())
		g_cl.m_cmd->m_view_angles.x = g_menu.main.config.mode.get() == 1 ? -180.f : 89.f;

	int side = invert ? -1 : 1;
	float angle = 125.f;

	if (g_menu.main.aimbot.yaw.get()) {
		if (!g_menu.main.aimbot.desync.get())
			g_cl.m_cmd->m_view_angles.y += 180.f;
		if (g_menu.main.aimbot.yaw.get() == 0) {
			if (*g_cl.m_packet)
			...
			if (*g_cl.m_packet)
				g_cl.m_cmd->m_view_angles.y += (angle + 30.f) * side;
			else
				g_cl.m_cmd->m_view_angles.y += (angle - 30.f) * side;
		}
	}
...
void HVH::SendPacket() {
	...
	if (g_aimbot.CanDT()) {
		*g_cl.m_packet = g_csgo.m_cl->m_choked_commands >= 1;
		...
		// fake-lag enabled.
		if (g_menu.main.aimbot.lag_enable.get() && !g_csgo.m_gamerules->m_bFreezePeriod() && !(g_cl.m_flags & FL_FROZEN)) {
			// limit of lag.
			int limit = std::min((int)g_menu.main.aimbot.lag_limit.get(), g_cl.m_max_lag);
			...
			auto activation = g_menu.main.aimbot.lag_active.GetActiveIndices();
			...
			if (active) {
				int mode = g_menu.main.aimbot.lag_mode.get();
		...
		if (!g_menu.main.aimbot.lag_land.get()) {
