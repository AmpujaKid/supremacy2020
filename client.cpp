...
void Client::DrawHUD() {
	...
	std::string text = tfm::format(XOR("leetgrail | rtt: %ims | %i"), ms, g_cl.m_user);
	render::FontSize_t size = render::hud.size(text);

	// background.
	render::rect_filled(m_width - size.m_width - 20, 10, size.m_width + 10, size.m_height + 2, { 0, 0, 0, 30 });
...
void Client::ClanTag()
{
	...
	std::string clantag = XOR("leetgrail ");
...
void Client::print(const std::string text, ...) {
	...
	// print to console.
	g_csgo.m_cvar->ConsoleColorPrintf(colors::burgundy, XOR("[leetgrail] "));
