...
public:
	...
	Dropdown      section; // this is the thing for the thing so that there aren't a billion
	// col1.
	Checkbox	  enable;
	Checkbox	  silent;
	Dropdown	  selection;
...
public:
	void init() {
		...
		enable.AddShowCallback(callbacks::IsSection0);
		RegisterElement(&enable);

		silent.setup(XOR("silent"), XOR("silent"));
		silent.AddShowCallback(callbacks::IsSection0);
		RegisterElement(&silent);

		selection.setup(XOR("target"), XOR("selection"), { XOR("distance"), XOR("crosshair"), XOR("damage"), XOR("health"), XOR("lag"), XOR("height") });
		selection.AddShowCallback(callbacks::IsSection0);
		RegisterElement(&selection);

		fov.setup(XOR("fov"), XOR("fov"));
		fov.AddShowCallback(callbacks::IsSection0);
		RegisterElement(&fov);
		...
		hitbox.AddShowCallback(callbacks::IsSection1);
		RegisterElement(&hitbox);

		hitbox_history.setup(XOR("backtrack"), XOR("hitbox_history"), { XOR("head"), XOR("chest"), XOR("body"), XOR("arms"), XOR("legs") });
		hitbox_history.AddShowCallback(callbacks::IsSection1);
		RegisterElement(&hitbox_history);

		multipoint.setup(XOR("multi-point"), XOR("multipoint"), { XOR("head"), XOR("chest"), XOR("body"), XOR("legs") });
		multipoint.AddShowCallback(callbacks::IsSection1);
		...
		scale.AddShowCallback(callbacks::IsSection1);
		...
		body_scale.AddShowCallback(callbacks::IsSection1);
		RegisterElement(&body_scale);

		minimal_damage.setup(XOR("min damage"), XOR("minimal_damage"), 1.f, 100.f, true, 0, 40.f, 1.f);
		minimal_damage.AddShowCallback(callbacks::IsSection1);
		RegisterElement(&minimal_damage);

		minimal_damage_hp.setup(XOR("scale damage on hp"), XOR("minimal_damage_hp"));
		minimal_damage_hp.AddShowCallback(callbacks::IsSection1);
		RegisterElement(&minimal_damage_hp);

		penetrate.setup(XOR("auto wall"), XOR("penetrate"));
		penetrate.AddShowCallback(callbacks::IsSection1);
		...
		penetrate_minimal_damage.AddShowCallback(callbacks::IsSection1);
		...
		penetrate_minimal_damage_hp.AddShowCallback(callbacks::IsSection1);
		RegisterElement(&penetrate_minimal_damage_hp);

		knifebot.setup(XOR("knifebot"), XOR("knifebot"));
		knifebot.AddShowCallback(callbacks::IsSection0);
		RegisterElement(&knifebot);

		zeusbot.setup(XOR("zeusbot"), XOR("zeusbot"));
		zeusbot.AddShowCallback(callbacks::IsSection0);
		RegisterElement(&zeusbot);

		enableaa.setup(XOR("anti aim"), XOR("enable"));
		enableaa.AddShowCallback(callbacks::IsSection2);
		RegisterElement(&enableaa);

		pitch.setup(XOR("pitch"), XOR("pitch"));
		pitch.AddShowCallback(callbacks::IsAntiAimOn);
		pitch.AddShowCallback(callbacks::IsSection2);
		RegisterElement(&pitch);

		yaw.setup(XOR("yaw"), XOR("yaw"));
		yaw.AddShowCallback(callbacks::IsAntiAimOn);
		yaw.AddShowCallback(callbacks::IsSection2);
		RegisterElement(&yaw);

		desync.setup(XOR("desync"), XOR("desync"), { XOR("static"), XOR("low delta") });
		desync.AddShowCallback(callbacks::IsAntiAimOn);
		desync.AddShowCallback(callbacks::IsSection2);
		RegisterElement(&desync);

		side_flip.setup(XOR("invert"), XOR("side_flip"));
		side_flip.SetToggleCallback(callbacks::ToggleDesyncSide);
		side_flip.AddShowCallback(callbacks::IsAntiAimOn);
		side_flip.AddShowCallback(callbacks::IsSection2);
		RegisterElement(&side_flip);

		fakeduck.setup(XOR("fake duck"), XOR("fakeduck"));
		fakeduck.AddShowCallback(callbacks::IsAntiAimOn);
		fakeduck.AddShowCallback(callbacks::IsSection2);
		RegisterElement(&fakeduck);

		// col2.
		section.setup(XOR("section"), XOR("section"), { XOR("main"), XOR("accuracy"), XOR("antiaim") });
		RegisterElement(&section, 1);

		zoom.setup(XOR("auto scope"), XOR("zoom"), { XOR("off"), XOR("always"), XOR("hitchance fail") });
		zoom.AddShowCallback(callbacks::IsSection1);
		RegisterElement(&zoom, 1);

		nospread.setup(XOR("compensate spread"), XOR("nospread"));
		nospread.AddShowCallback(callbacks::IsConfigNS);
		nospread.AddShowCallback(callbacks::IsSection1);
		RegisterElement(&nospread, 1);

		norecoil.setup(XOR("compensate recoil"), XOR("norecoil"));
		norecoil.AddShowCallback(callbacks::IsSection1);
		RegisterElement(&norecoil, 1);

		hitchance.setup(XOR("hitchance"), XOR("hitchance"));
		hitchance.AddShowCallback(callbacks::IsConfigMM);
		hitchance.AddShowCallback(callbacks::IsSection2);
		...
		hitchance_amount.AddShowCallback(callbacks::IsSection2);
		RegisterElement(&hitchance_amount, 1);

		correction.setup(XOR("correction"), XOR("correction"), { XOR("fake-lag"), XOR("anti-aim") });
		correction.AddShowCallback(callbacks::IsSection0);
		RegisterElement(&correction, 1);

		baim1.setup(XOR("prefer baim"), XOR("baim1"), { XOR("always"), XOR("lethal"), XOR("lethal x2"), XOR("fake"), XOR("in air") });
		baim1.AddShowCallback(callbacks::IsSection1);
		RegisterElement(&baim1, 1);

		baim2.setup(XOR("only baim"), XOR("baim2"), { XOR("always"), XOR("health"), XOR("fake"), XOR("in air") });
		baim2.AddShowCallback(callbacks::IsSection1);
		...
		baim_hp.AddShowCallback(callbacks::IsSection1);
		RegisterElement(&baim_hp, 1);

		baim_key.setup(XOR("baim override"), XOR("body aim on key"));
		baim_key.AddShowCallback(callbacks::IsSection1);
		RegisterElement(&baim_key, 1);

		override_dmg_key.setup(XOR("override damage"), XOR("override_dmg"));
		override_dmg_key.AddShowCallback(callbacks::IsSection1);
		...
		override_dmg_value.AddShowCallback(callbacks::IsSection1);
		RegisterElement(&override_dmg_value, 1);

		autostop_always_on.setup(XOR("automatic stop always on"), XOR("auto_stop_always"));
		autostop_always_on.AddShowCallback(callbacks::IsSection0);
...
public:
	void init() {
		...
		viewmodel_fov.setup(XOR("viewmodel fov"), XOR("viewmodel_fov"));
...
public:
	...
	void init() {
		...
		config.setup(XOR("configuration"), XOR("cfg"), { XOR("auto"), XOR("scout"), XOR("awp"), XOR("rifles"), XOR("heavy"), XOR("pistol") });
